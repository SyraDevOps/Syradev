...
def safe_eval(expr):
    ...
    # PRIORIDADE 0: Verificar se é um comando registrado com prefixo '&'
    command_call_match = re.match(r"&(\w+)\s*\((.*)\)", expr)
    if command_call_match:
        command_name = command_call_match.group(1)
        command_args = command_call_match.group(2)
        cmd_key = f"&{command_name}"
        ...
        if cmd_key in commands:
            return commands[cmd_key](command_args)
        
        cmd_key_lower = f"&{command_name.lower()}"
        if cmd_key_lower in commands:
            return commands[cmd_key_lower](command_args)

    # PRIORIDADE 1: Verificar se é uma função Syra
    call_match = re.match(r"(\$?\w+)\s*\((.*)\)", expr)
    if call_match:
        func_name = call_match.group(1)
        params_str = call_match.group(2)
        if func_name in syra_def.syra_functions:
            func_obj = syra_def.syra_functions[func_name]
            args = []
            kwargs = {}
            if params_str.strip():
                param_parts = params_str.split(',')
                for part in param_parts:
                    ...
                    if '=' in part:
                        ...
                        args.append(safe_eval(part.strip()))
            return func_obj(*args, **kwargs)

    # PRIORIDADE 2: Verificar se é uma variável Syra
    if expr.startswith("$"):
        return variables.get(expr)

    # PRIORIDADE 3: Avaliar como expressão Python
    try:
...
def run_syra_file(filename):
    ...
    while i < len(lines_from_file):
        ...
        if "//" in line_for_logic: # Remove comentário inline para decisão
        ...
        if line_for_logic.startswith("class "):
            ...
            base_indent_class = len(lines_from_file[i]) - len(lines_from_file[i].lstrip())
            processed_idx = i + 1
            while processed_idx < len(lines_from_file):
                next_line_raw = lines_from_file[processed_idx]
                # Para verificação de indentação, não remova comentários ainda, apenas strip
                next_line_stripped_for_check = next_line_raw.strip() 
                
                current_indent = len(next_line_raw) - len(next_line_raw.lstrip())
                
                # Linha vazia ou mais indentada pertence ao bloco da classe
                if not next_line_stripped_for_check or current_indent > base_indent_class:
        ...
        # --- Function Definition Block (handles decorators too) ---
            ...
            func_def_block_lines = []
            
            current_block_line_idx = i
            header_line_of_function_proper_raw = None 
            indent_of_function_header = -1

            # 1. Coletar decoradores e a linha de cabeçalho da função
            while current_block_line_idx < len(lines_from_file):
                line_to_add_raw = lines_from_file[current_block_line_idx].rstrip('\n')
                line_to_add_cleaned_for_logic = line_to_add_raw.strip()
                if "//" in line_to_add_cleaned_for_logic:
                    line_to_add_cleaned_for_logic = line_to_add_cleaned_for_logic.split("//", 1)[0].strip()

                if not line_to_add_cleaned_for_logic and not func_def_block_lines: 
                    # Pular linhas vazias ANTES da primeira linha de definição (decorador ou cabeçalho)
                    current_block_line_idx += 1
                    if i == current_block_line_idx -1 : i = current_block_line_idx # Atualiza i principal se pulou
                    continue
                
                # Se já começamos a coletar (func_def_block_lines não está vazio) E a linha atual está vazia,
                # ela pode ser parte do espaçamento interno do bloco de definição (ex: entre decoradores).
                # Ou se for a primeira linha e for um decorador/cabeçalho.
                is_decorator = line_to_add_cleaned_for_logic.startswith("@")
                is_func_header = re.match(r"^\w+\s*\(.*\)\s+is(\s|:|->)", line_to_add_cleaned_for_logic)

                if is_decorator:
                    func_def_block_lines.append(line_to_add_raw)
                    current_block_line_idx += 1
                elif is_func_header:
                    func_def_block_lines.append(line_to_add_raw)
                    header_line_of_function_proper_raw = line_to_add_raw
                    indent_of_function_header = len(header_line_of_function_proper_raw) - len(header_line_of_function_proper_raw.lstrip())
                    current_block_line_idx += 1
                    break 
                elif func_def_block_lines and not line_to_add_cleaned_for_logic: # Linha vazia após já ter coletado algo
                    func_def_block_lines.append(line_to_add_raw) # Manter linha vazia
                    current_block_line_idx +=1
                else: 
                    # Não é decorador, nem cabeçalho de função, nem linha vazia após início de coleta.
                    # O bloco de definição anterior (se houver) terminou, ou esta linha é inválida aqui.
                    break 
            
            # 2. Coletar corpo da função (se o cabeçalho terminar com ':' e tivermos um cabeçalho)
            if header_line_of_function_proper_raw and \
               header_line_of_function_proper_raw.strip().split("//",1)[0].strip().endswith(":") and \
               indent_of_function_header != -1:
                
                while current_block_line_idx < len(lines_from_file):
                    next_body_line_raw = lines_from_file[current_block_line_idx].rstrip('\n')
                    
                    if not next_body_line_raw.strip(): # Linha vazia no corpo é permitida
                        func_def_block_lines.append(next_body_line_raw)
                        current_block_line_idx += 1
                        continue

                    current_body_indent = len(next_body_line_raw) - len(next_body_line_raw.lstrip())
                    
                    if current_body_indent > indent_of_function_header:
                        func_def_block_lines.append(next_body_line_raw)
                        current_block_line_idx += 1
                    else:
                        break 
            
            if func_def_block_lines:
                syra_def.define_syra_function("\n".join(func_def_block_lines))
            
            i = current_block_line_idx 
            continue

        # --- Lambda assignment (geralmente uma linha, não precisa de lógica de bloco complexa aqui) ---
        elif re.match(r"\$\w+\s*=\s*\(.*\)\s+is\s+.+", line_for_logic):
            # ... (código existente para lambda, sem alterações na coleta de bloco) ...
        ...
        # --- Match Block ---
            ...
            base_indent_match = len(lines_from_file[i]) - len(lines_from_file[i].lstrip())
            processed_idx = i + 1
            while processed_idx < len(lines_from_file):
                next_line_raw = lines_from_file[processed_idx]
                ...
                if not next_line_stripped_for_check or \
                   next_line_stripped_for_check.lower().startswith("case ") or \
                   current_indent > base_indent_match:
        ...
        # --- Each Block ---
            ...
            base_indent_each = len(lines_from_file[i]) - len(lines_from_file[i].lstrip())
            processed_idx = i + 1
            while processed_idx < len(lines_from_file):
                next_line_raw = lines_from_file[processed_idx]
                next_line_stripped_for_check = next_line_raw.strip().split("//",1)[0].strip()
                ...
                if not next_line_stripped_for_check or current_indent > base_indent_each:
